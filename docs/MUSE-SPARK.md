...
These are the details that would otherwise fail at runtime, each taken from
Meta's Responses API documentation:
...
- **Assistant text before a tool call must be tagged `phase: "commentary"`.**
  Replaying it as an ordinary final answer immediately before a `function_call`
  is a 400; the server will not silently reinterpret it. The adapter derives
  the phase from whether tool calls follow, so it survives the round trip
...
- **`reasoning_content` does not exist on Responses output** — that field
  ...
  `reasoning.summary: "auto"` and synthesizes `reasoning_content` from the
  summary. Without that the chat UI's reasoning panel would simply be empty.
  Set `LLM_REASONING_SUMMARY = null` to save those tokens and accept a blank
  panel.
...
The adapter is built against Meta's published Responses API documentation and
has unit coverage for every translation step, including a full tool-calling
round trip (`test/llm-providers.test.js`). It has **not** been exercised
against the live Meta API — that needs `MODEL_API_KEY`, which only you can set.
`/admin/llm-selftest` is one request and it exercises the real code path; run
it before the first real chat turn.
