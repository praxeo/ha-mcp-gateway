...
| Voice-reply style (short, narrative, no lists) | The `voiceTurn` block in `renderDynamicContext()` — injected when the turn originated from the browser mic (`source: "voice"`) **or the UI's spoken-replies toggle is on** (`speak: true`, forwarded by the worker and DO `/ai_chat` routes). Tells the model the reply will be read aloud and must be a couple of spoken-prose sentences. |
