...
POST = SSE chat), `/dash` (GET — cover states + glanceable house tiles for the
chat UI's dashboard strip), `/covers` (GET — legacy subset of `/dash`, kept for
cached pages), `/twilio` (dormant), `/mcp` (and `/` — MCP JSON-RPC), plus
