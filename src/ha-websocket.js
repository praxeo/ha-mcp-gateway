...
import { trimChatHistory, byteLength, DO_VALUE_LIMIT_BYTES } from "./chat-history.js";
...
// Reserved top-level keys on a call_service / schedule_action tool call.
// Everything else at the top level is treated as a stray service parameter
// and folded into the nested data object (see coalesceServiceData).
const SERVICE_CALL_RESERVED_KEYS = new Set([
  "domain", "service", "data", "service_data", "target",
  "return_response", "delay_seconds", "description"
]);

// Some models (notably MiniMax M3) emit service parameters — entity_id,
// temperature, brightness, etc. — at the TOP LEVEL of a call_service /
// schedule_action tool call instead of nesting them inside `data` /
// `service_data`. The dispatch and the scheduler only read the nested
// object, so those flat params were silently dropped and HA rejected the
// call ("must contain at least one of entity_id, ..." /
// "must contain at least one of temperature, ..."). This folds any stray
// top-level key into the nested data object; explicit nested values win on
// conflict. entity_id is also lifted out of a `target` object so the
// scheduler — which always fires with target:{} — still sees it.
...
// ===========================================================================
// Runtime LLM config — resolution, request shaping, and validation
//
// The chat/agent model, endpoint, and reasoning mode are runtime-configurable
// so a model swap (e.g. Qwen ↔ MiniMax) does NOT require the DO-rename
// migration dance (CLAUDE.md gotcha #1). Three layers, highest precedence
// first:
//   1. DO storage override  — set live via the /llm_config route (no deploy)
//   2. env vars             — LLM_ENDPOINT / LLM_MODEL / LLM_REASONING_MODE /
//                             LLM_REASONING_EFFORT (picked up by a fresh isolate)
//   3. baked-in defaults    — HAWebSocketV34._defaultLLMConfig()
// These helpers are pure and module-level for unit testing; keep the stub in
// test/llm-config.test.js in sync.
// ===========================================================================

const LLM_REASONING_MODES = new Set(["thinking", "effort", "none"]);
// Union across providers; each provider clamps to what it accepts at call
// time (clampEffortForProvider). Fireworks takes none/low/medium/high; Meta
// adds minimal and xhigh.
const LLM_REASONING_EFFORTS = PROVIDER_EFFORTS;

// Pull the env-var layer into a partial config. Only keys actually present in
// env contribute; everything else falls through to the defaults.
...
// Layer the stored override over the env layer over the defaults. `stored` and
// `defaults` are full/partial config objects; `env` is the raw env bag. Returns
// a complete, validated effective config plus a `source` map noting where each
// field came from (for the /llm_config diagnostic view).
function resolveLLMConfig(stored, env, defaults) {
  ...
  const api = pick("api");
  // Guard against a stored/env value that slipped a bad enum (e.g. config
  // written by an older schema) — fall back to the default for that field.
  ...
  // An endpoint that was never set explicitly follows the provider/api pair,
  // so switching provider is one field and not three.
  const explicitEndpoint = endpoint.source === "default" ? null : endpoint.value;
  const safeEndpoint = endpointFor(safeProvider, safeApi, explicitEndpoint)
    || base.endpoint;
  return {
...
// Mutate an OpenAI-compatible request body with the reasoning control implied
// by the resolved config, ensuring `thinking` and `reasoning_effort` are never
// both present (Fireworks 400s on that). Returns the same body for chaining.
function applyReasoningToBody(body, cfg) {
  ...
  // mode === "none" (or anything else) → send neither.
  return body;
}

// Validate + normalize a config patch coming from the /llm_config route. Only
// known keys are accepted; unknown keys and bad types/enums are rejected.
...
function sanitizeLLMConfigPatch(patch) {
  ...
  for (const [k, v] of Object.entries(patch)) {
    if (k === "reset") continue; // handled by the caller
...
export class HAWebSocketV34 {
  ...
  // LLM provider — OpenAI-compatible chat completions on Fireworks. These are
  // the BAKED-IN DEFAULTS only. The effective config is resolved at call time by
  // resolveLLMConfig (DO storage override → env vars → these defaults), so the
  ...
  // reasoning_effort:<LLM_REASONING_EFFORT>; "none" → neither. We drive reasoning
  // via reasoning_effort (mode "effort"). Fireworks treats reasoning_effort and
  // the Anthropic-style thinking toggle as interchangeable and maps between them
  // (and maps NIM enable_thinking onto reasoning_effort), so "high" works across
  ...
  static _defaultLLMConfig() {
    return {
      endpoint: HAWebSocketV34.LLM_ENDPOINT,
      model: HAWebSocketV34.LLM_MODEL,
      provider: HAWebSocketV34.LLM_PROVIDER,
      api: HAWebSocketV34.LLM_API,
      reasoning_mode: HAWebSocketV34.LLM_REASONING_MODE,
      reasoning_effort: HAWebSocketV34.LLM_REASONING_EFFORT
    };
  }
  // Fireworks prompt caching is replica-local. A fixed session-affinity key
  // pins all gateway traffic to one serverless replica so the byte-identical
  // static prefix (system prompt + tool schemas) stays warm across turns and
  // tool-loop iterations. Routing hint only — does not change model output.
  // Ref: Fireworks "Prompt caching → Optimizing inference request for caching".
  static SESSION_AFFINITY_KEY = "ha-mcp-gateway";
  ...
  static _tsFromMs(ms) {
    ...
    return {
      fired_at_ms: ms,
      fired_at_iso: isoTs,
      fired_at_central: HAWebSocketV34._formatTimelineTimestamp(isoTs)
    };
  }
  ...
  _buildHouseStateSnapshot() {
    ...
    const GROUPS = {
      ...
      "TESLA MODEL Y": [
        ...
        "sensor.tesla_model_y_battery_range",
        "sensor.tesla_model_y_charging",
        "binary_sensor.tesla_model_y_charge_cable",
        "lock.tesla_model_y_lock",
        "device_tracker.tesla_model_y_location",
        "binary_sensor.tesla_model_y_status",
        "sensor.tesla_model_y_inside_temperature",
      ]
    };
  ...
  constructor(state, env) {
    ...
    // Runtime LLM config — the DO-storage override layer, lazily loaded once on
    // first LLM call (and refreshed in-process when set via /llm_config).
    this._llmConfigStored = null;
    this._llmConfigLoaded = false;
  }

  // Resolve the effective LLM config (override → env → defaults). The stored
  // override is read from DO storage once and cached in-process; the /llm_config
  // route updates the cache when it writes, so a config change goes live on the
  // running isolate with no rename/redeploy.
  async _getLLMConfig() {
    if (!this._llmConfigLoaded) {
      this._llmConfigStored = (await this.state.storage.get("llm_config")) || null;
      this._llmConfigLoaded = true;
    }
    return resolveLLMConfig(this._llmConfigStored, this.env, HAWebSocketV34._defaultLLMConfig());
  }
...
LAYOUT NOTE — non-obvious: 3 levels with walkout basement; there is no upstairs. If asked anything room-specific you can't resolve from RELEVANT ENTITIES or the LOCK/GARAGE MAP below, call get_house_topology before answering.
...
`;
  ...
  // ==========================================================================
  // HOUSE TOPOLOGY — full room-by-room layout. Moved behind the
  // get_house_topology tool in V10 to keep the default system prompt small.
  // The LOCK MAP, GARAGE/BAY DOOR mappings, and KEY DEVICES disambiguation
  // tables stay in getAgentContext() since they fire on every cover/lock
  // command. This text is retrieved on demand when the model needs to
  // resolve room-specific queries it can't answer from the inline tables.
  // ==========================================================================
  _getHouseTopologyText() {
    return `THE HOUSE:
...
Exception: when the user explicitly says "remember X" or "save a memory" or equivalent, save it regardless of the above criteria. Always confirm the save verbatim in your reply.`;
  ...
  // V10: gates HOUSE_STATE_SNAPSHOT (locks / covers / climate / presence /
  // power / contact sensors / modes). Matches aggregate-state, security,
  // and presence terms — plus the household's first names so "did Sabrina
  // leave?" / "is John home?" loads the snapshot without an extra get_state
  // round-trip. Liberal on purpose: false positive = include snapshot when
  // maybe not needed (cheap); false negative = model has no presence
  // context (worse).
  static HOUSE_STATUS_TRIGGER_RE = /\b(?:house|status|secure|everything|all\s+(?:lock|door|cover|light|window)s?|anything\s+(?:open|unlocked|running|on)|summary|going\s+on|weird|alarm|armed|intrusion|burglar|home|away|gone|left|back|arrived|outside|tesla|vehicle|car|john|sabrina)\b/i;

  static climateTriggerMatches(text) {
    if (!text || typeof text !== "string") return false;
    return HAWebSocketV34.CLIMATE_TRIGGER_RE.test(text);
  }

  static houseStatusTriggerMatches(text) {
    if (!text || typeof text !== "string") return false;
    return HAWebSocketV34.HOUSE_STATUS_TRIGGER_RE.test(text);
  }
  ...
  async _buildClimatePreambleIfNeeded(triggerText, source = "chat") {
    ...
    if (!HAWebSocketV34.climateTriggerMatches(triggerText)) return null;
    ...
    const seasonStr = HAWebSocketV34._seasonDominant(monthIdx);

    const tempStr = (w.temperature !== null && w.temperature !== undefined) ? `${w.temperature}°F` : "n/a";
    const condStr = w.state || "unknown";

    const hl = HAWebSocketV34._forecastHighLow(w.forecast);
    const trend = HAWebSocketV34._forecastTrend(w.forecast);
...
${fmtZone("Main", "climate.t6_pro_z_wave_programmable_thermostat_2", c.main)}`;
  ...
  async fetch(request) {
    ...
    if (!this._logInitialized) {
      ...
      if (this.aiLog.length > HAWebSocketV34.LOG_IN_MEMORY_CAP) {
        this.aiLog = this.aiLog.slice(-HAWebSocketV34.LOG_IN_MEMORY_CAP);
      }
    ...
    try {
      switch (url.pathname) {
        ...
        // Runtime LLM config. GET returns the effective config (with a per-field
        // source map), the stored override, and the baked-in defaults. POST sets
        // the stored override ({ reset: true } clears it). Changes take effect on
        // the next LLM call with no rename/redeploy — the in-process cache is
        // updated here so even this pinned isolate picks them up immediately.
        case "/llm_config": {
          if (request.method === "POST") {
            const reqBody = await request.json().catch(() => null);
            if (reqBody && reqBody.reset === true) {
              await this.state.storage.delete("llm_config");
              this._llmConfigStored = null;
              this._llmConfigLoaded = true;
            } else {
              const result = sanitizeLLMConfigPatch(reqBody);
              if (result.error) {
                return new Response(JSON.stringify({ error: result.error }), { status: 400, headers });
              }
              // Ensure we merge onto whatever is already stored, not a stale null.
              if (!this._llmConfigLoaded) {
                this._llmConfigStored = (await this.state.storage.get("llm_config")) || null;
              }
              const merged = { ...(this._llmConfigStored || {}), ...result.value };
              await this.state.storage.put("llm_config", merged);
              this._llmConfigStored = merged;
              this._llmConfigLoaded = true;
            }
          }
          const effective = await this._getLLMConfig();
          return new Response(JSON.stringify({
            effective,
            stored_override: this._llmConfigStored || null,
            env_layer: llmConfigFromEnv(this.env),
            defaults: HAWebSocketV34._defaultLLMConfig()
          }, null, 2), { headers });
        }

        // Prove a provider/model actually works BEFORE the chat path — and
        // therefore the house — depends on it. Sends one tiny request with one
        // dummy tool and reports what came back: whether text arrived, whether
        // a tool call round-trips in the expected shape, whether reasoning is
        // returned, plus token usage and latency. Accepts an optional config
        // patch to test a candidate WITHOUT storing it, so a bad model never
        // becomes the live default.
        case "/llm_selftest": {
          const patch = request.method === "POST"
            ? await request.json().catch(() => null)
            : null;
          let cfg = await this._getLLMConfig();
          if (patch && Object.keys(patch).length) {
            const parsed = sanitizeLLMConfigPatch(patch);
            if (parsed.error) {
              return new Response(JSON.stringify({ error: parsed.error }), { status: 400, headers });
            }
            // Resolve the candidate over the stored layer without persisting.
            cfg = resolveLLMConfig(
              { ...(this._llmConfigStored || {}), ...parsed.value },
              this.env,
              HAWebSocketV34._defaultLLMConfig()
            );
          }
          if (!this._apiKeyFor(cfg)) {
            return new Response(JSON.stringify({
              ok: false,
              error: `${cfg.api_key_env} not configured`,
              hint: `wrangler secret put ${cfg.api_key_env}`,
              config: cfg
          ...
          const probeTools = [{
            type: "function",
            function: {
              name: "get_house_temperature",
              description: "Return the current indoor temperature in Fahrenheit.",
              parameters: {
                type: "object",
                properties: { floor: { type: "string", description: "main or basement" } },
                required: ["floor"]
              }
            }
          }];
          const probeMessages = [
            { role: "system", content: "You are a test harness. Use the provided tool when asked about temperature." },
            { role: "user", content: "What is the temperature on the main floor? Use the tool." }
          ];

          // Swap the resolved config in for the duration of the probe so
          // callLLMWithTools exercises the real code path, not a copy of it.
          const savedStored = this._llmConfigStored;
          const savedLoaded = this._llmConfigLoaded;
          const t0 = Date.now();
          let out;
          try {
            if (patch && Object.keys(patch).length) {
              this._llmConfigStored = { ...(savedStored || {}), ...sanitizeLLMConfigPatch(patch).value };
              this._llmConfigLoaded = true;
            }
            const data = await this.callLLMWithTools(probeMessages, probeTools, 512, 30000);
            const msg = data?.choices?.[0]?.message || {};
            const calls = Array.isArray(msg.tool_calls) ? msg.tool_calls : [];
            out = {
              ok: true,
              elapsed_ms: Date.now() - t0,
              config: { provider: cfg.provider, api: cfg.api, model: cfg.model, endpoint: cfg.endpoint, reasoning_mode: cfg.reasoning_mode, reasoning_effort: cfg.reasoning_effort },
              returned_text: typeof msg.content === "string" ? msg.content.slice(0, 300) : null,
              tool_call_count: calls.length,
              tool_call_shape_ok: calls.length > 0
                && typeof calls[0].id === "string"
                && typeof calls[0].function?.name === "string"
                && typeof calls[0].function?.arguments === "string",
              first_tool_call: calls[0] || null,
              reasoning_returned: !!msg.reasoning_content,
              reasoning_chars: (msg.reasoning_content || "").length,
              reasoning_replay_items: Array.isArray(msg._reasoning_items) ? msg._reasoning_items.length : 0,
            ...
            out = {
              ok: false,
              elapsed_ms: Date.now() - t0,
              config: { provider: cfg.provider, api: cfg.api, model: cfg.model, endpoint: cfg.endpoint },
              error: err.message
            };
          } finally {
            this._llmConfigStored = savedStored;
            this._llmConfigLoaded = savedLoaded;
          }
          return new Response(JSON.stringify(out, null, 2), { status: out.ok ? 200 : 502, headers });
        }
        ...
        case "/ai_log": {
          ...
          if (count > HAWebSocketV34.LOG_IN_MEMORY_CAP) {
        ...
        case "/ai_chat": {
          const body = await request.json();
          const response = await this.chatWithAgent(body.message, body.from || "default", null, { tier: body.tier, source: body.source, speak: body.speak === true });
        ...
        case "/ai_chat_stream": {
          ...
          }, { tier: body.tier, source: body.source, speak: body.speak === true }).then(() => {
        ...
        // Ephemeral one-shot scheduler — MCP parity. Bypasses executeNativeTool
        // so the request can carry source: "mcp" through to the storage record
        // (executeNativeTool hardcodes "chat" for the native loop path).
        case "/schedule_action": {
          const body = await request.json().catch(() => ({}));
          const result = await this._scheduleAction({
            delaySeconds: body.delay_seconds,
            domain: body.domain,
            service: body.service,
            serviceData: body.service_data || {},
        ...
        case "/list_scheduled_actions": {
          const result = await this._listScheduledActions();
          return new Response(JSON.stringify(result), { headers });
        }
        case "/cancel_scheduled_action": {
          const body = await request.json().catch(() => ({}));
          const result = await this._cancelScheduledAction({ id: body.task_id });
          return new Response(JSON.stringify(result), { headers });
        }
  ...
  onEvent(event) {
    ...
    if (event.event_type === "automation_triggered" && event.data) {
      ...
      const { fired_at_ms, fired_at_iso, fired_at_central } =
        HAWebSocketV34._tsFromMs(Date.parse(event.time_fired) || Date.now());
  ...
  _serializeStateCacheForSnapshot() {
    ...
    for (const [id, s] of this.stateCache) {
      ...
      if (!HAWebSocketV34.SNAPSHOT_DOMAIN_ALLOWLIST.has(domain)) continue;
      ...
      if (domain === "sensor") {
        ...
        if (HAWebSocketV34.SENSOR_WHITELIST.has(deviceClass)) {
          // keep
        } else if (deviceClass === "battery") {
          const pct = parseFloat(s.state);
          if (isNaN(pct) || pct > HAWebSocketV34.BATTERY_LOW_THRESHOLD) continue;
        } else {
          continue;
        }
      }
      ...
      const filteredAttrs = {};
      for (const k of HAWebSocketV34.SNAPSHOT_ATTR_ALLOWLIST) {
    ...
    for (const group of groups) {
      ...
      for (let i = 1; i < group.length; i++) {
        ...
        this._writeStateChangeToD1({
          ...
          fired_at_central: HAWebSocketV34._formatTimelineTimestamp(new Date(tsMs).toISOString()),
          context_id: null,
  ...
  async _migrateRetireAiLogKeys() {
    try {
      ...
      for (let i = 0; i < HAWebSocketV34.LOG_CHUNKS_MAX; i++) {
  ...
  async _writeAiLogEntryToD1(entry) {
    ...
    try {
      ...
      if (this.aiLog.length > HAWebSocketV34.LOG_IN_MEMORY_CAP) {
        this.aiLog.splice(0, this.aiLog.length - HAWebSocketV34.LOG_IN_MEMORY_CAP);
      }
    }
  }
  ...
  // Reduce echoed-back assistant messages to the fields the API accepts as
  // input: role, content, tool_calls, and reasoning_content. Strips
  // response-only junk (audio_content, reasoning, ...) that strict input
  // validation rejects. reasoning_content IS kept — Fireworks requires it
  // echoed back on prior assistant turns for interleaved thinking across
  // tool-call rounds.
  static sanitizeMessagesForLLM(messages) {
    return messages.map((m) => {
      if (!m || m.role !== "assistant") return m;
      const clean = { role: "assistant", content: m.content };
      ...
      // Responses-API reasoning objects, replayed verbatim on the next round
      // of the same turn. The chat-format branch never sends these; the
      // adapter consumes them and history strips them (stripProviderInternals).
      if (Array.isArray(m._reasoning_items) && m._reasoning_items.length) {
        clean._reasoning_items = m._reasoning_items;
      }
      return clean;
    });
  }
  ...
  // ========================================================================
  // Capture Fireworks server-side perf metrics. Non-streaming responses carry
  // them as response headers (the perf_metrics_in_response flag is streaming-
  // only), so grab every fireworks-* header — server TTFT, prompt/cached
  // tokens — letting a slow turn be split into prefill (cache miss) vs
  // generation. Headers is iterable in the Workers runtime.
  // ========================================================================
  static _extractPerfHeaders(headers) {
    const perf = {};
    try {
      for (const [k, v] of headers) {
        if (k.toLowerCase().startsWith("fireworks-")) perf[k.toLowerCase()] = v;
      }
    } catch (_) {}
    return perf;
  }

  // Normalize the raw fireworks-* headers into the chat_timing_ms field names:
  // server_ttft_ms (prefill — dominated by prompt-cache hit vs miss),
  // server_total_ms (server processing time, when the header is present), and
  // perf_cached_tokens (cross-checks the body's cached_tokens). Compare
  // server_ttft_ms against fireworks_ms (wall-clock) to split a slow turn into
  // prefill vs generation. Raw headers stay in `perf` so the unit can be
  // audited against a live response.
  static _normalizePerf(perf) {
    const p = perf || {};
    return {
      server_ttft_ms: HAWebSocketV34._parsePerfMs(p["fireworks-server-time-to-first-token"]),
      server_total_ms: HAWebSocketV34._parsePerfMs(p["fireworks-server-processing-time"]),
      perf_cached_tokens: HAWebSocketV34._parseIntOrNull(p["fireworks-cached-prompt-tokens"])
    };
  }

  // Fireworks doesn't document the perf header time unit. Realistic LLM TTFT /
  // processing values are sub-100 in seconds and >=100 in ms, so this yields
  // the right millisecond value either way for realistic latencies; the raw
  // header is retained in `perf` for verification.
  static _parsePerfMs(v) {
  ...
  // ========================================================================
  // LLM API helper — Fireworks, OpenAI-compatible endpoint
  ...
  async callLLM(messages, maxTokens = 32768, jsonMode = false) {
    ...
    if (!apiKey) throw new Error(`${label}: ${cfg.api_key_env} not configured`);
    // This legacy, tool-free path always speaks Chat Completions. When the
    // configured wire format is "responses", target the same provider's chat
    // endpoint rather than posting a chat body to a responses URL.
    const endpoint = cfg.api === "responses"
      ? (endpointFor(cfg.provider, "chat", null) || cfg.endpoint)
      : cfg.endpoint;
    const body = {
      model: cfg.model,
      messages: HAWebSocketV34.sanitizeMessagesForLLM(messages),
    ...
    // guarantees at most one is set. Reasoning returns in reasoning_content.
    ...
    if (cfg.provider === "fireworks") {
      _headers["x-session-affinity"] = HAWebSocketV34.SESSION_AFFINITY_KEY;
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(`${label} API ${response.status}: ${errText.substring(0, 200)}`);
    }
    const data = await response.json();
    data._perf = HAWebSocketV34._extractPerfHeaders(response.headers);
    const msg = data.choices?.[0]?.message;
    if (msg) {
  ...
  async loadLogFromStorage() {
    const fromD1 = await this._loadAiLogFromD1(HAWebSocketV34.LOG_IN_MEMORY_CAP);
  ...
  async chatWithAgent(message, from = "default", onEvent = null, opts = {}) {
    ...
    const timeline = persistentLog
      ...
      .slice(-150)
      .map((e) => `[${HAWebSocketV34._formatTimelineTimestamp(e.timestamp)}] ${e.type}: ${e.message}`)
      .join("\n");
    ...
    for (const [id, state] of this.stateCache) {
      ...
      if (HAWebSocketV34.CONTEXT_DOMAIN_PRIORITY.includes(domain)) {
        ...
        if (HAWebSocketV34.SENSOR_WHITELIST.has(deviceClass)) {
          ...
          include = !isNaN(pct) && pct <= HAWebSocketV34.BATTERY_LOW_THRESHOLD;
        }
        if (include) {
    ...
    let sensorCount = 0;
    for (const domain of [...HAWebSocketV34.CONTEXT_DOMAIN_PRIORITY, "sensor"]) {
      for (const entry of (byDomain.get(domain) || [])) {
        if (contextEntities.length >= HAWebSocketV34.MAX_CONTEXT_ENTITIES) break;
        if (domain === "sensor") {
          if (sensorCount >= HAWebSocketV34.MAX_SENSOR_CONTEXT) break;
          sensorCount++;
        }
...
Emit ONE JSON object. No markdown fences. No text outside the JSON. If nothing to do, use "actions": [].`;
    ...
    const attemptChat = async (previousAssistantResponse = null, correction = null) => {
      ...
      if (!responseText) {
        ...
        const jsonFallback = HAWebSocketV34.extractFirstJSON(rawReasoning);
        if (jsonFallback) responseText = jsonFallback;
      }
      let parsed = null;
      const jsonMatch = HAWebSocketV34.extractFirstJSON(responseText);
      if (jsonMatch) {
        try {
      ...
      switch (name) {
        ...
        case "call_service":
          return await this.executeAIAction({
            ...
            data: coalesceServiceData(args, "data"),
            target: args.target || {},
        ...
        case "schedule_action":
          return await this._scheduleAction({
            delaySeconds: args.delay_seconds,
            domain: args.domain,
            service: args.service,
            serviceData: coalesceServiceData(args, "service_data"),
            description: args.description || "",
            source: "chat"
          });

        case "cancel_scheduled_action":
          return await this._cancelScheduledAction({ id: args.task_id });

        // ----- Read tools (no action log; not counted in actions_taken) -----
        case "list_scheduled_actions":
          return await this._listScheduledActions();
        ...
        case "get_house_topology": {
          // V10: on-demand topology — text is large (~2k tokens) so it
          // doesn't ship in the default system prompt. Returned verbatim
          // from _getHouseTopologyText(). Zero-arg, no validation.
          return { topology: this._getHouseTopologyText() };
        }
  ...
  // ========================================================================
  // Persist a chat history value, trimmed so it fits the DO's 131072-byte
  // per-value limit. The reply has already been delivered by the time this
  // runs, so a failure here is invisible to the user and only shows up as the
  // agent having forgotten the conversation on the NEXT turn — which is
  // exactly how this went unnoticed for a while. Hence: trim first, verify the
  // size, and keep a degraded fallback rather than letting the write throw.
  async _persistChatHistory(key, messages) {
    let trimmed;
    try {
      trimmed = trimChatHistory(messages);
    } catch (err) {
      this.logAI("history_trim_error", err.message, { key }, "chat_history");
      trimmed = [];
    }
    try {
      ...
      // Salvage the current exchange rather than losing the thread entirely.
      const lastUser = trimmed.map((m) => m.role).lastIndexOf("user");
      const salvage = trimChatHistory(
        lastUser === -1 ? trimmed.slice(-2) : trimmed.slice(lastUser),
        { maxTurns: 1, byteBudget: 16000 }
      );
      this.logAI(
        "history_put_failed",
        `${err.message} | bytes=${byteLength(trimmed)} limit=${DO_VALUE_LIMIT_BYTES} → salvaged ${salvage.length} msgs`,
        { key, messages: trimmed.length, bytes: byteLength(trimmed), salvaged: salvage.length },
        "chat_history"
      );
      try {
        await this.state.storage.put(key, salvage);
        return salvage;
      } catch (err2) {
        this.logAI("history_put_failed_final", err2.message, { key }, "chat_history");
        return null;
      }
    }
  }

  // Resolve the API key for whichever provider the config selects. Returns
  // null when the key isn't configured, so callers can fail with a message
  // naming the missing secret rather than a bare 401 from the provider.
  _apiKeyFor(cfg) {
    const name = (cfg && cfg.api_key_env) || "FIREWORKS_API_KEY";
  ...
  async callLLMWithTools(messages, tools, maxTokens = 16384, timeoutMs = 45000, effortOverride = null) {
    ...
    const clean = HAWebSocketV34.sanitizeMessagesForLLM(messages);
    const isResponses = cfg.api === "responses";
    ...
    if (isResponses) {
      ...
      body = buildResponsesBody({
        ...
        summary: HAWebSocketV34.LLM_REASONING_SUMMARY,
        cacheKey: HAWebSocketV34.SESSION_AFFINITY_KEY
      });
    } else {
      body = {
        model: cfg.model,
        // Responses-API reasoning objects are meaningless to a chat-completions
        // provider and would be rejected as unknown fields, so they never leave
        // this branch. (They only exist at all after a provider switch inside a
        // live isolate.)
        messages: clean.map(stripProviderInternals),
        tools,
        max_tokens: maxTokens,
        temperature: 0
      };
    ...
    // Replica pinning is a Fireworks routing hint for its prompt cache; it is
    // meaningless elsewhere, so don't send it to other providers.
    if (cfg.provider === "fireworks") {
      headers["x-session-affinity"] = HAWebSocketV34.SESSION_AFFINITY_KEY;
    }
    ...
    try {
      ...
      const data = isResponses ? responsesToChatCompletion(raw) : raw;
      data._perf = HAWebSocketV34._extractPerfHeaders(response.headers);
      return data;
    } catch (err) {
      if (err.name === "AbortError") {
        throw new Error(`${label} API timeout after ${timeoutMs}ms`);
      }
      throw err;
    } finally {
      clearTimeout(timeout);
  ...
  async runNativeToolLoop(initialMessages, options = {}) {
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
      ...
      const _perf = response._perf || {};
      // Fireworks prompt prefix cache — tokens served from cache this
      // iteration. Climbs on iterations 2+ once the static system+tools
      // prefix is warm (and, with x-session-affinity pinning the replica, on
      // iteration 1 of a follow-up turn that re-hits it). Default 0 if absent.
      ...
      // Keep reasoning_content on echoed assistant messages — Fireworks needs
      // prior-turn reasoning re-fed for interleaved thinking across tool-call
      // rounds. Strip only the legacy `reasoning` alias (unused by Fireworks).
      const { reasoning, ...assistantEcho } = assistantMsg;
      ...
      if (toolCalls.length === 0) {
        ...
        _meta.per_iteration.push({
          iter,
          fireworks_ms: _modelEnd - _modelStart,
          tool_exec_ms: 0,
          tool_names: [],
          ...
          perf: _perf,
          ...HAWebSocketV34._normalizePerf(_perf)
        });
      ...
      // save_memory, save_observation, report_bug all stay on the LLM
      // wrap-up path because their replies benefit from context.
      if (toolCalls.length === 1) {
        const fastTc = toolCalls[0];
        if (fastTc.function?.name === "call_service") {
          let fastArgs = {};
          try { fastArgs = JSON.parse(fastTc.function?.arguments || "{}"); } catch (_) {}
          const fastKey = `${fastArgs.domain || ""}.${fastArgs.service || ""}`;
          const POST_TOOL_FAST_KEYS = new Set([
            "light.turn_on", "light.turn_off", "light.toggle",
            "switch.turn_on", "switch.turn_off", "switch.toggle",
            "lock.lock", "lock.unlock",
            "scene.turn_on"
          ]);
          // The just-pushed tool message has the result as a JSON string.
          const lastToolMsg = messages[messages.length - 1];
          let toolErrored = false;
          try {
            const r = lastToolMsg && typeof lastToolMsg.content === "string"
              ? JSON.parse(lastToolMsg.content)
              : null;
            toolErrored = !!(r && typeof r === "object" && r.error);
          } catch (_) { toolErrored = false; }
          const modelEmittedText = stripThink(assistantMsg.content || "").trim();
          if (POST_TOOL_FAST_KEYS.has(fastKey) && !toolErrored && !modelEmittedText) {
            const entityId = fastArgs.entity_id || fastArgs.target?.entity_id || null;
            let friendly = entityId;
            if (entityId && this.stateCache.has(entityId)) {
              friendly = this.stateCache.get(entityId)?.attributes?.friendly_name || entityId;
            }
            const VERB = {
              "light.turn_on": "on",
              "light.turn_off": "off",
              ...
              "switch.turn_on": "on",
              "switch.turn_off": "off",
              "switch.toggle": "toggled",
              "lock.lock": "locked",
              "lock.unlock": "unlocked",
              "scene.turn_on": "activated"
            };
            const verb = VERB[fastKey] || "done";
            const synthReply = friendly ? `Done — ${friendly} ${verb}.` : `Done.`;
            messages.push({ role: "assistant", content: synthReply });
            safeEmit({ type: "reply", text: synthReply });
            return {
              reply: synthReply,
              actions_taken: actionsTaken,
              messages,
              iterations: iter + 1,
              synthesized_post_tool: true,
              _meta
            };
          }
        }
    ...
    try {
      ...
      _meta.per_iteration.push({
        iter: maxIterations,
        fireworks_ms: _synthEnd - _synthStart,
        tool_exec_ms: 0,
        tool_names: [],
        ...
        perf: _synthPerf,
        ...HAWebSocketV34._normalizePerf(_synthPerf),
        synthesis: true
      });
  ...
  async _buildNativeContextEntities(query = null, options = {}) {
    ...
    if (useVector && typeof query === "string" && query.trim().length > 0) {
      try {
        ...
        // pull a few automations so action queries ("turn on the dock light")
        // have the relevant automation pre-injected. V21: devices/services
        // retrieval dropped — rarely matched, low value; vector_search covers
        // them on demand.
        const [
          ...
          this.retrieveKnowledge({ query, k: 3, kinds: ["memory"], includeNoisy: false }),
          this.retrieveKnowledge({ query, k: 3, kinds: ["observation"], includeNoisy: false }),
          this.retrieveKnowledge({ query, k: 2, kinds: ["automation"], includeNoisy: false })
        ]);
  ...
  _buildNativeTimeline() {
    // V9 triage: chat_user and chat_reply entries are excluded because they
    // duplicate the OpenAI `conversation_history` messages array — the model
    // was previously re-reading its own multi-paragraph past replies in prose
    // on every turn, costing ~5k duplicate tokens. Action/state/snapshot
    // events are the load-bearing content. V21: window last 1 hour OR last
    // 15 events, whichever is tighter (prompt trim).
    const relevantTypes = ["action", "action_verified", "notification", "decision", "state_change", "memory_saved", "observation_saved"];
    ...
    return this.aiLog
      ...
      .slice(-15)
      .map((e) => `[${HAWebSocketV34._formatTimelineTimestamp(e.timestamp)}] ${e.type}${e.source ? "/" + e.source : ""}: ${e.message}`)
      .join("\n");
  }
  ...
  getStaticChatSystemPrompt() {
    return buildStaticChatSystemPrompt(this.getAgentContext());
  }
  ...
  buildDynamicContext(ctx) {
    const { message = "", contextEntities = [] } = ctx;
    // HOUSE_STATE_SNAPSHOT gating (~830 tokens): include when the query matches
    // the status/presence/security regex, vector retrieval returned no
    // high-confidence entity, or no message was passed (legacy callers).
    ...
    const includeSnapshot =
      !message ||
      HAWebSocketV34.houseStatusTriggerMatches(message) ||
...
`
      : "";
    return renderDynamicContext({ ...ctx, snapshot, healthBlock });
  }
  ...
  async chatWithAgentNative(message, from = "default", onEvent = null, opts = {}) {
    ...
    const _t1 = Date.now();
    if (fastResult) {
      // V12: fire the reply event FIRST. The HA service call already
      // completed inside _tryDeterministicFastPath — the garage is
      // already moving. Holding the SSE event behind ~30-70ms of
      // bookkeeping (logAI / storage.get+put / persistLog) was pure
      // user-visible latency. Now: emit the reply, then do bookkeeping
      // before returning. The SSE stream stays open until return, but
      ...
      try {
        ...
        await this._persistChatHistory(fpHistoryKey, fpNext);
      } catch (_) {}
      {
    ...
    } = await this._buildNativeContextEntities(message, { entityTopK: 6 });
    const _t3 = Date.now();
    ...
    const dynamicContext = this.buildDynamicContext({
      ...
      semanticServices,
      from,
      voiceTurn: opts.source === "voice" || opts.speak === true,
    ...
    // trim against the literal prompt rather than estimates.
    if (this.env.DUMP_SYSTEM_PROMPT === "1") {
      try {
        const _allowed = NATIVE_AGENT_TOOLS.filter(
          (t) => CHAT_ALLOWED_TOOL_NAMES.has(t.function.name)
        );
        const _breakdown = {
          system_prompt_chars: systemPrompt.length,
          history_msg_count: conversationHistory.length,
          history_total_chars: JSON.stringify(conversationHistory).length,
          user_msg_chars: message.length,
          tool_count: _allowed.length,
          tools_total_chars: JSON.stringify(_allowed).length,
          per_tool: _allowed.map((t) => ({
            name: t.function.name,
            desc_chars: (t.function.description || "").length,
            params_chars: JSON.stringify(t.function.parameters || {}).length,
            total_chars: JSON.stringify(t).length
          })),
          full_messages_chars: JSON.stringify(initialMessages).length
        };
        this.logAI(
          "system_prompt_breakdown",
          `messages=${_breakdown.full_messages_chars}c tools=${_breakdown.tools_total_chars}c sys=${_breakdown.system_prompt_chars}c hist=${_breakdown.history_total_chars}c`,
          _breakdown,
          "preflight"
        );
        this.logAI(
        ...
        // Preflight diagnostics must never break a chat turn.
      }
    }
    ...
    try {
      ...
      const result = await this.runNativeToolLoop(initialMessages, {
        ...
        hallucinationGuard: true,
        maxTokens: 4096,
        // Quick (the UI default, reset on every page load) reasons at
        // "minimal"; High is the per-session opt-in for forensic and trend
        // questions. See LLM_TIERS in llm-providers.js.
      ...
      // Content caps and the reasoning strip both happen in trimChatHistory
      // now; this pass only drops the provider-bound replay blobs, which must
      // not survive into the array that gets reused.
      const loopAdditions = result.messages.slice(1 + oldHistoryLen + 1).map((m) => (
        m.role === "assistant" ? stripProviderInternals(m) : m
      ));
      ...
      await this._persistChatHistory(historyKey, nextHistory);
      this.logAI(
        "chat",
  ...
  // ========================================================================
  // Ephemeral one-shot scheduler. Records live in DO storage under the
  // `scheduled:<uuid>` namespace and are fired by `_fireDueScheduledActions`
  // on the 60s alarm tick (see alarm() below). Two delivery rules:
  //   - Delete-BEFORE-fire: better to lose a task on a transient failure than
  //     to fire it twice (e.g. a "set_temperature" replay would be fine but a
  //     "toggle" would silently undo itself).
  //   - WS-up guard: skip the tick entirely when the HA WS is down; the next
  //     tick after reconnect picks the task up. Avoids destroying a task to a
  //     transient outage.
  // ========================================================================
  async _scheduleAction({ delaySeconds, domain, service, serviceData, description, source = "chat" }) {
    const d = Number(delaySeconds);
    if (!Number.isFinite(d) || d <= 0) {
      return { error: "delay_seconds must be a positive number" };
    }
    if (d > 60 * 60 * 24 * 30) {
      return { error: "delay_seconds cannot exceed 30 days (2592000s)" };
    }
    ...
    this.logAI(
      ...
      `${record.description || `${domain}.${service}`} in ${Math.round(d)}s`,
      { id, fire_at_ms: fireAtMs, domain, service, source: record.source },
      "scheduler"
    );
    return {
      ok: true,
      id,
      fire_at_ms: fireAtMs,
      fires_in_seconds: Math.round((fireAtMs - now) / 1000),
  ...
  async _listScheduledActions() {
    let map;
    try {
      map = await this.state.storage.list({ prefix: "scheduled:" });
    } catch (err) {
      return { error: "Failed to list scheduled tasks: " + err.message };
    }
    const now = Date.now();
    const tasks = [];
    for (const [, v] of map) {
      ...
      tasks.push({
        ...
        service_call: v.service_call || {},
        fire_at_ms: v.fire_at_ms,
        fires_in_seconds: Math.max(0, Math.round((v.fire_at_ms - now) / 1000)),
  ...
  async _cancelScheduledAction({ id }) {
    ...
    this.logAI(
      ...
      existing.description || `${existing.service_call?.domain}.${existing.service_call?.service}`,
      { id, fire_at_ms: existing.fire_at_ms },
      "scheduler"
    );
  ...
  async _fireDueScheduledActions() {
    ...
    for (const [key, task] of due) {
      ...
      if (!domain || !service) {
        this.logAI("scheduled_task_failed", `missing domain/service for task ${task.id}`,
          { id: task.id }, "scheduler");
        continue;
      }
      try {
        ...
        this.logAI(
          ...
          task.description || `${domain}.${service}`,
          { id: task.id, domain, service, fired_after_due_ms: now - task.fire_at_ms },
          "scheduler"
        );
      } catch (err) {
        this.logAI(
          "scheduled_task_failed",
          `${domain}.${service} — ${err.message}`,
          { id: task.id, error: String(err?.message || err) },
          "scheduler"
        );
      }
    }
  ...
  // ========================================================================
  // Alarm handler — WS keepalive + ephemeral scheduler tick. Reschedules
  // itself unconditionally. The reschedule is MANDATORY — it's a redundant
  // DO keepalive alongside the persistent HA WebSocket and the minute-cadence
  // prewarmCache cron. Without it, the DO can hibernate when the WS goes
  // quiet. Do not remove.
  // ========================================================================
  async alarm() {
    if (!this.connected || !this.authenticated) {
    ...
    try {
      await this._fireDueScheduledActions();
    } catch (err) {
      try { this.logAI("scheduler_tick_failed", err.message, { error: String(err?.message || err) }, "scheduler"); } catch {}
    }
